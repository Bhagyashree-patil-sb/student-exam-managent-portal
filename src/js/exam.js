...
let timerDuration = 1200;  // Timer for Part A (20 minutes)
let timerInterval;
...
function startTimer() {
    timerInterval = setInterval(function() {
        ...
        if (timerDuration < 0 && !partAComplete) {
            ...
            timerDuration = 600;  // Timer for Part B (10 minutes)
            loadQuestion(currentQuestionIndex);
            startTimer();  // Restart the timer for Part B
        }
...
function loadQuestion(index) {
    ...
    updateProgressBar();  // Update progress bar each time a question is loaded
...
function isValidJavaSumCode(code) {
    ...
    return score === correctCode.length ? 7 : score;
}
...
function isValidJavaCalculatorCode(code) {
    ...
    return score === correctCode.length ? 10 : score;
}
